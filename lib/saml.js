...
function setup() {
  ...
  host(function(err, host) {
    ...
    samlConfig = {
      ...
      authnContext: process.env.SAML_AUTHN_CONTEXT || 'urn:oasis:names:tc:SAML:2.0:ac:classes:PasswordProtectedTransport',
...
router.get('/login/ok', ensureAuthenticated, function(req, res) {
  var params = {
    ...
    groups: lookupFromProfile('groups', req.user)? lookupFromProfile('groups', req.user) : [],
